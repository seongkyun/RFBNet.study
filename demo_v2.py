...
if __name__ == '__main__':
    ...
    #Detect_test(num_classes, bkg_label, cfg, nms_th, conf_th, max_det=100)
    #detector = Detect_test(num_classes,0,cfg)
    ...
    _labels, _scores, _coords = object_detector.predict(img)
    ...
    for labels, scores, coords in zip(_labels, _scores, _coords):
        ...
        cv2.putText(img, '{label}: {score:.3f}'.format(label=COCO_CLASSES[labels], score=scores), (int(coords[0]), int(coords[1])), FONT, 0.5, COLORS[labels % 3], 2)

    cv2.imwrite('temp_result.jpg', img)
